<div>
    ...
    <x-page-header :title="$project->name" :description="$project->description">
        ...
        @can('archive', $project)<flux:button wire:click="archive" variant="ghost" icon="archive-box">{{ $project->status === 'archived' ? 'Restore' : 'Archive' }}</flux:button>@endcan
    </x-page-header>
    ...
    <div class="mb-6 flex gap-1 overflow-x-auto border-b border-zinc-200 dark:border-zinc-800">
        ...
        @can('update', $project)<button wire:click="$set('tab', 'settings')" class="border-b-2 px-4 py-3 text-sm font-medium {{ $tab === 'settings' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-zinc-500' }}">Settings</button>@endcan
    </div>
    ...
    @if($tab === 'documents')
        <div class="grid gap-6 xl:grid-cols-[1fr_340px]">
            <section class="overflow-hidden rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
                ...
                @if($documents->count())<div class="divide-y divide-zinc-100 dark:divide-zinc-800">@foreach($documents as $document)<a href="{{ route('documents.show', [$project, $document]) }}" wire:navigate class="flex items-center gap-4 px-6 py-4 hover:bg-zinc-50 dark:hover:bg-zinc-800/50"><span class="grid size-10 place-items-center rounded-lg bg-zinc-100 text-zinc-500 dark:bg-zinc-800"><flux:icon.document-text class="size-5" /></span><span class="min-w-0 flex-1"><span class="block truncate text-sm font-medium">{{ $document->name }}</span><span class="block text-xs text-zinc-500">{{ $document->owner->name }} · {{ $document->human_size }} · {{ $document->updated_at->diffForHumans() }}</span></span><x-status-badge :value="$document->visibility" /><flux:icon.chevron-right class="size-4 text-zinc-400" /></a>@endforeach</div>@else<div class="p-6"><x-empty-state icon="document-plus" title="No documents yet" description="Upload the first document for this project." /></div>@endif
            </section>
            @can('create', [\App\Models\Document::class, $project])
                @if($project->status === 'active')
                    <form wire:submit="upload" class="h-fit rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
                        <h2 class="font-semibold">Upload document</h2><p class="mt-1 text-sm text-zinc-500">Add a file to this workspace.</p>
                        <div class="mt-5 space-y-4"><flux:input wire:model="file" type="file" label="File" /><flux:input wire:model="documentName" label="Display name" placeholder="Optional" /><flux:select wire:model="visibility" label="Visibility"><flux:select.option value="project">Everyone in project</flux:select.option><flux:select.option value="private">Private</flux:select.option></flux:select></div>
                        <flux:button type="submit" variant="primary" class="mt-5 w-full" wire:loading.attr="disabled">Upload</flux:button>
                    </form>
