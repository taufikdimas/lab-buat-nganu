<div>
    <x-page-header title="Projects" description="Plan work, share documents, and keep every decision in one place."><flux:button variant="primary" icon="plus" :href="route('projects.create')" wire:navigate>Create project</flux:button></x-page-header>
    ...
    @if($projects->count())
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            @foreach($projects as $project)
                ...
                <a href="{{ route('projects.show', $project) }}" wire:navigate class="group rounded-2xl border border-zinc-200 bg-white p-5 transition hover:-translate-y-0.5 hover:border-indigo-300 hover:shadow-lg hover:shadow-indigo-950/5 dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-indigo-700">
    ...
    @else
        <x-empty-state icon="folder-plus" title="No projects found" description="Create a project to bring your team and documents together."><flux:button variant="primary" :href="route('projects.create')" wire:navigate>Create project</flux:button></x-empty-state>
    @endif
</div>
