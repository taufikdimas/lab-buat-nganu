...
<body class="min-h-screen bg-zinc-50 text-zinc-950 dark:bg-zinc-950 dark:text-zinc-100">
    <flux:sidebar sticky stashable class="border-r border-zinc-200/80 bg-white dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-2 py-2" wire:navigate>
        ...
        <flux:navlist variant="outline" class="mt-6">
            <flux:navlist.group heading="Workspace" class="grid">
                <flux:navlist.item icon="home" :href="route('dashboard')" :current="request()->routeIs('dashboard')" wire:navigate>Overview</flux:navlist.item>
                <flux:navlist.item icon="folder" :href="route('projects.index')" :current="request()->routeIs('projects.*')" wire:navigate>Projects</flux:navlist.item>
                <flux:navlist.item icon="magnifying-glass" :href="route('search')" :current="request()->routeIs('search')" wire:navigate>Search</flux:navlist.item>
            ...
            @if(auth()->user()->isAdmin())
                <flux:navlist.group heading="Administration" class="mt-5 grid">
                    <flux:navlist.item icon="shield-check" :href="route('admin.index')" :current="request()->routeIs('admin.index')" wire:navigate>Admin overview</flux:navlist.item>
                    ...
                    <flux:navlist.item icon="folder-open" :href="route('admin.projects')" :current="request()->routeIs('admin.projects')" wire:navigate>All projects</flux:navlist.item>
                    <flux:navlist.item icon="document-text" :href="route('admin.documents')" :current="request()->routeIs('admin.documents')" wire:navigate>All documents</flux:navlist.item>
                    <flux:navlist.item icon="clipboard-document-list" :href="route('admin.audit-logs')" :current="request()->routeIs('admin.audit-logs')" wire:navigate>Audit logs</flux:navlist.item>
                    <flux:navlist.item icon="cog-6-tooth" :href="route('admin.settings')" :current="request()->routeIs('admin.settings')" wire:navigate>Settings</flux:navlist.item>
                </flux:navlist.group>
            @endif
        </flux:navlist>
        <flux:spacer />
        <flux:dropdown position="bottom" align="start">
            ...
            <flux:menu class="w-60">
                ...
                <flux:menu.separator />
                <flux:menu.item :href="route('profile')" icon="user-circle" wire:navigate>Profile</flux:menu.item>
                <flux:menu.item href="/settings/password" icon="key" wire:navigate>Security</flux:menu.item>
                <flux:menu.item href="/settings/appearance" icon="moon" wire:navigate>Appearance</flux:menu.item>
                <flux:menu.separator />
